...
def post_list_view(request):
     ...
     return render(request,'BlogApp/post_list.html',{'postlist':postlist})
...
def post_list_view(request,tag_slug=None):
    ...
    return render(request,'BlogApp/post_list.html',{"postlist":postlist,'tag':tag})
...
def mailsendview(request,id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            send_mail(Subject,message,'uppalayashwanth904@gmail.com',[cd['to']])
            sent =True
    else:
         form = Emailsendform()
...
def thankyou1(request):
    return render(request, 'BlogApp/thankyou1.html')
